...
- Dedicated static blog article pages exist at `/blog/{slug}/`; the legacy popup remains available for backward compatibility.
...
## Dedicated Page and Popup Architecture

Blogs are rendered as cards in the homepage `#blogs` section. Each card keeps the existing visual design, but its primary action calls `openBlogArticle(index)` and navigates to `/blog/{slug}/` when a valid slug exists.

Dedicated article pages are generated by `blog/generator/blog-page-generator.js` from Google Sheets data through Apps Script. They live at:
...
The legacy popup remains available as a fallback.

When the fallback popup opens:
...
↓
Frontend navigates to `/blog/{slug}/` when the slug is valid
↓
Static article page displays full content
```

## Current Limitations

- Some published CMS rows are skipped from generated output when the `Slug` field is missing or invalid.
- Blog generation is currently a manual build step, not an automated CI workflow.
- Projects, certificates, experience, categories, and tags do not yet have generated static pages.
- Google Docs blog content is converted with simple text replacement, not rich semantic HTML conversion.
- The legacy popup still fetches content at runtime for fallback behavior.
...
- Automate the static generation build in CI.
- Extend generation to projects, certificates, experience, categories, and tags.
- Add richer validation for broken images, broken links, duplicate metadata, and scheduled publishing.
...
Current `sitemap.xml` includes the homepage, blog index, and generated blog article URLs:

```text
https://itsmebillah.github.io/
https://itsmebillah.github.io/blog/
https://itsmebillah.github.io/blog/{slug}/
```
...
- Some published Blog rows are skipped from static output until valid slugs are provided.
- No generated static pages exist yet for projects, certificates, or experience.
...
- Build-time validators exist for generated blog metadata, schema, sitemap, RSS, and search index, but broader broken-link and image validation should be expanded.
...
- Generated blog pages support unique metadata, canonical URLs, Open Graph, Twitter Cards, BlogPosting JSON-LD, and breadcrumb schema.

Future improvement:

- Extend per-page metadata generation to projects, certificates, experience, categories, and tags.
...
The first generator layer is implemented for blogs:

- `blog/generator/blog-page-generator.js`
- `blog/generator/sitemap-generator.js`
- `blog/generator/rss-generator.js`
- `blog/generator/search-index-generator.js`
- `blog/templates/blog.template.html`
- `blog/templates/seo.template.html`
